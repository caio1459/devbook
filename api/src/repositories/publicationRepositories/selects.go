...
func (p publicationRepositorie) SelectPublication(id uint64) (models.Publications, error) {
	row, err := p.db.Query(`
		SELECT p.title, p.content, p.image_url, p.likes, u.nick, u.user_id, p.register
	...
	if row.Next() {
		if err = row.Scan(
			&publication.Title, &publication.Content, &publication.ImageUrl, &publication.Likes,
			&publication.ActorNick, &publication.UserID, &publication.Register,
		); err != nil {
			return models.Publications{}, err
		}
	}
...
func (p publicationRepositorie) SelectPublications(id uint64) ([]models.Publications, error) {
	rows, err := p.db.Query(`
		SELECT DISTINCT p.title, p.content, p.image_url, p.likes, u.nick, p.register
		FROM publications p
	...
	for rows.Next() {
		publication := models.Publications{}
		if rows.Scan(
			&publication.Title, &publication.Content, &publication.ImageUrl,
			&publication.Likes, &publication.ActorNick, &publication.Register,
		); err != nil {
			return nil, err
		}
...
func (p publicationRepositorie) SelectPublicationsFromUser(id uint64) ([]models.Publications, error) {
	rows, err := p.db.Query(`
		SELECT p.title, p.content, p.image_url, p.likes, p.user_id, u.nick, p.register 
		FROM publications p, users u
	...
	for rows.Next() {
		...
		if err = rows.Scan(
			&publication.Title, &publication.Content, &publication.ImageUrl, &publication.Likes,
			&publication.UserID, &publication.ActorNick, &publication.Register,
		); err != nil {
			return nil, err
		}
		publications = append(publications, publication)
